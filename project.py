...
def showRecipes(cat_id):
    ...
    logged_in = loggedIn()
    if 'username' not in login_session:
      return render_template('publicrecipes.html', category = category, items = items, logged_in = logged_in)
    else:
      return render_template('recipes.html', category = category, items = items, logged_in = logged_in)
...
def showRecipe(cat_id, rec_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
      return render_template('publicrecipe.html', recipe = recipe, cat_id = cat_id, creator = creator, logged_in = logged_in)
    else:
      return render_template('recipe.html', recipe = recipe, cat_id = cat_id, creator = creator, logged_in = logged_in)
...
def newRecipe(cat_id):
  ...
  if request.method == 'POST':
      newRecipe = Recipe(name = request.form['name'], ingredients = request.form['ingredients'], directions = request.form['directions'], picture = request.form['picture'], cat_id = cat_id, creator_id = creator_id)
      session.add(newRecipe)
      session.commit()
      flash('New Recipe %s Successfully Created' % (newRecipe.name))
      return redirect(url_for('showRecipes', cat_id = cat_id, logged_in = logged_in))
  else:
      return render_template('newrecipe.html', cat_id = cat_id, logged_in = logged_in)

#Edit a recipe
@app.route('/categories/<int:cat_id>/<int:rec_id>/edit', methods=['GET','POST'])
def editRecipe(cat_id, rec_id):
    ...
    if editedItem.creator.username != login_session['username']:
      return "<script>function myFunction() {alert('you are not authorized to edit this recipe. please create your own recipe.');}</script><bodyonload='myFunction()''>" 
    if request.method == 'POST':
        ...
        return render_template('editrecipe.html', rec_id = rec_id, cat_id = cat_id, item = editedItem, logged_in = logged_in)


#Delete a recipe
@app.route('/categories/<int:cat_id>/<int:rec_id>/delete', methods = ['GET','POST'])
def deleteRecipe(cat_id, rec_id):
    ...
    if itemToDelete.creator.username != login_session['username']:
      return "<script>function myFunction() {alert('you are not authorized to delete this recipe. please create your own recipe.');}</script><bodyonload='myFunction()''>" 
    if request.method == 'POST':
        ...
        return render_template('deleterecipe.html', item = itemToDelete, cat_id = cat_id, logged_in = logged_in)
